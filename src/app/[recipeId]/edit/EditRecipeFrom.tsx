...
import { updateRecipeSchema } from "@/schemas/recipeSchema";
import { DifficultyLevel } from "@prisma/client";
import type { Category, Recipe } from "@prisma/client";
import type { updateRecipeSchemaType } from "@/schemas/recipeSchema";
...
export default function EditRecipeForm({
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<updateRecipeSchemaType>({
    defaultValues: {
      recipeId: recipe?.id,
      title: recipe?.title,
  ...
  const { mutate, isLoading } = api.recipe.updateRecipe.useMutation({
    onSuccess: (data) => {
      toast.success("recipe updated");
      reset();
      router.push(`/${data?.id}/edit/ingredients`);
    },
    onError: (error) => toast.error(error.message),
  });

  const onSubmit = (data: updateRecipeSchemaType) => {
    mutate(data);
  };
